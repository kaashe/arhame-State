...
import { useCreateListingMutation, useGetSingleListingQuery } from '../store/api/listingSlice';
import { useSelector } from 'react-redux';
import { useNavigate, useParams } from 'react-router-dom';

const CreateListing = () => {
    const { id } = useParams();
    const { data: listingSingleData, isLoading: singleListIsFetching } = useGetSingleListingQuery(id);
    console.log(listingSingleData, 'ddddd');
                    ...
                    <div className="flex flex-col flex-1 gap-4">
                        ...
                        {id && <div>
                            ...
                            <ul className='flex gap-2'>
                                {listingSingleData?.imageUrls?.map((imageUrl, index) => (
                                    <li key={index}>
                                        {/* Display the image */}
                                        <img
                                            src={imageUrl}
                        ...
                        <button disabled={isCreateListingLoading} type="submit" className='mt-3 w-96 py-1 rounded bg-green-600 text-white' value="">
                            <span className='flex items-center justify-center gap-2 my-auto'>
                                <span>{id ? "Update Listing" : "Create Listing"}</span>
